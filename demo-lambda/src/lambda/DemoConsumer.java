...
public class DemoConsumer {
  public static void main(String[] args) {
    Consumer<String> sendEmailFuntion = emailAddress -> sendEmail(emailAddress);

    List<String> emails = new ArrayList<>(List.of(""));
    emails.forEach(sendEmailFuntion);

    // Common way to use Funtion
    emails.forEach(x -> {
      sendEmail(x);
    });
    ...
    // for each
    books.forEach((e -> { // e (element)= list of book
    ...
    // See if it SuperConsumer after extends it. (Can use FuntionalInterface)
